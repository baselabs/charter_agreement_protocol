...
| Identity | Value | Job |
|---|---|---|
| Corpus digest | `sha-256:sLJmUSpiuPoAjoiynGzdNSntQ4V-NV7p0zfeoIsUmaA` | Domain-separated digest inside the canonical corpus index; proves index self-consistency |
| Index SHA-256 (base64url) | `NiSzeS8F0SXS6ddeeQhOBdsG4BQn8jcxb8DSX1q-oLM` | Raw SHA-256 of the exact canonical index bytes; pinned by the Elixir CLI and the TypeScript verifier core |
...
| Specification digest | `sha-256:O9nzLxQyq6vRb6QrRaos1Se0WG2Whd7BDaIWRvpCTe8` | Domain-separated digest over the canonical manifest of the normative spec set (`spec/`); pinned in release metadata |
...
case files under `cases/` (57 certified cases total). Each case file is
...
same report serialization, byte-identical to Elixir on all 57 cases.
